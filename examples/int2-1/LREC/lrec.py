...
def parse_args():
    ...
    parser.add_argument('--base_model', type=str, default='decapoda-research/llama-7b-hf', help='The base model.')
    parser.add_argument('--intq_checkpoint', type=str, default='llama7b-2bit-128g.pt', help='The intq checkpoint.')
    parser.add_argument('--wbits', type=int, default=2, help='The number of bits for weight quantization.')
    parser.add_argument('--groupsize', type=int, default=128, help='The group size.')
    parser.add_argument('--lora_alpha', type=int, default=128, help='The Lora alpha value.')
    parser.add_argument('--lora_r', type=int, default=32, help='The Lora r value.')
    parser.add_argument('--lora_dropout', type=float, default=0.05, help='The Lora dropout rate.')
    parser.add_argument(
        '--lora_target_modules',
        nargs='+',
        default=[
    ...
    parser.add_argument('--n_samples', type=int, default=2048, help='The number of samples.')
    ...
    parser.add_argument('--kl_weight', type=float, default=1., help='The KL weight.')
    parser.add_argument('--ce_weight', type=float, default=200., help='The CE weight.')
    parser.add_argument('--trainable_kl_weight', action='store_true', help='Whether to learn the KL weight.', default=False)
    parser.add_argument('--trainable_ce_weight', action='store_true', help='Whether to learn the CE weight.', default=False)
    parser.add_argument('--weight_decay', type=float, default=1e-5, help='The weight decay.')
    parser.add_argument('--save_freq', type=int, default=1, help='The frequency (period) of saving checkpoints in epochs.')
    parser.add_argument('--intra_save_freq', type=int, default=200, help='The period (in num_batches) of saving checkpoints within an epoch.')
    parser.add_argument('--seed', type=int, default=0, help='The random seed.')
    parser.add_argument('--seqlen', type=int, default=2048, help='The sequence length.')
    parser.add_argument('--cache', action='store_true', default=True, help='Use cached distillation outputs.')
    parser.add_argument('--train_cache_dir', type=str, default='###ANONYMIZED###/train_cache/', help='Training cache directory.')
    parser.add_argument('--val_cache_dir', type=str, default='###ANONYMIZED###/val_cache/', help='Validation cache directory.')
    parser.add_argument('--ckpt_dir', type=str, default='', help='The directory for saving and loading checkpoints.')
    parser.add_argument('--save_dir', type=str, default='', help='The directory for saving the final model.')
...
def prepare_models(args):
    if not args.cache:
        fp_model = AutoModelForCausalLM.from_pretrained(args.base_model, torch_dtype=torch.float16).to('cuda')
    ...
    if reduction == 'batchmean':
        if pointwise_loss.dtype == torch.float16:
            ...
            if torch.isinf(pointwise_loss.sum()):
                loss = torch.tensor([0], dtype=torch.float16, device=pointwise_loss.device)
...
def train_model(args, model, fp_model, trainloader, valenc):
    ...
    base_model_name = args.intq_checkpoint  + '-qer-r' + str(args.lora_r) + '-tm' + str(args.lora_target_modules) + '-ce' + str(args.ce_weight) + '-kl' + str(args.kl_weight) + '-lr' + str(args.lr) + '-bs' + str(args.batch_size) + '-wd' + str(args.weight_decay) + '-dstl' + str(args.train_cache_dir.split('/')[-2])
    ...
    def get_cached_targets(batch_idx, batch_size, cache_dir):
        ...
        return torch.cat(targets, dim=0)  # Concatenate targets along the batch dimension
    ...
    def eval_model(model, valenc, train_flag=False, max_samples=256):
        ...
        with torch.no_grad():
            ...
            for batch_idx, (inp, labels) in val_pbar:
                ...
                if args.cache:
                    pre_target = get_cached_targets(batch_idx, args.batch_size, args.val_cache_dir if not train_flag else args.train_cache_dir)
                    target = pre_target.add(1e-7).log_softmax(dim=-1) 
                else:
                    target = fp_model(input_ids=inp, labels=labels).logits.log_softmax(dim=-1)
                ...
                val_pbar.set_description(f'KL loss: {kl_loss.item():.3f} | CE loss: {ce_loss.item()}')
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        train_loss, train_kl_loss, train_ce_loss = eval_model(model, trainloader, train_flag=True)
        wandb.log({'Epoch': epoch, 'Train loss': train_loss, 'Validation loss': val_loss,
                     'Train KL loss': train_kl_loss, 'Validation KL loss': val_kl_loss,
                        'Train CE loss': train_ce_loss, 'Validation CE loss': val_ce_loss}, step=epoch * len(trainloader))
        ...
        for batch_idx, (inp, labels) in pbar:
            ...
            model_out_logsoftmax = model_out.add(1e-7).log_softmax(dim=-1)
            if args.cache:
                target = get_cached_targets(batch_idx, args.batch_size, args.train_cache_dir).add(1e-7).log_softmax(dim=-1)
            else:
                with torch.no_grad():
                    target = fp_model(input_ids=inp, labels=labels).logits.log_softmax(dim=-1)
            ...
            wandb.log({"Epoch": epoch, "Batch": batch_idx, "CE loss": ce_loss.item(), 
                       "KL loss": kl_loss.item(), "Total loss": loss.item()}, step=epoch*len(trainloader) + batch_idx)
            optimizer.zero_grad()
            ...
            if batch_idx % args.intra_save_freq == 0 and batch_idx != 0:
                with torch.no_grad():
                    ...
                    torch.save(ckpt, f'{args.save_dir}/tmp/ckpts/' + base_model_name + f'-{epoch}-{batch_idx}.pt')
                    model.save_pretrained(f'{args.save_dir}/tmp/models/{base_model_name}-{epoch}-{batch_idx}')
        ...
        if epoch % args.save_freq == 0:
            ...
            model.save_pretrained(f'{args.save_dir}/tmp/models/{base_model_name}-{epoch}')
...
def main():
    ...
    trainloader, valenc = get_c4(args.base_model, args.seqlen, args.n_samples, args.batch_size, args.seed)
    if args.cache:
