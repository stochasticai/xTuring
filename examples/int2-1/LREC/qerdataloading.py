...
def get_c4(base_model, seqlen, n_samples, batch_size, seed=0):
    ...
    trainloader = create_random_trainloader(traindata, tokenizer, seqlen, n_samples, seed)
    valenc = create_random_valenc(valdata, tokenizer, seqlen, seed)

    trainloader = DataLoader(trainloader, batch_size=batch_size, shuffle=False, drop_last=True)
