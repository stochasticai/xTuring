...
from qerdataloading import load_c4_datasets, create_random_trainloader, create_random_valenc

HF_CACHE_DIR = ... ###ANONYMIZED###

def cache_distillation_outputs(base_model, seqlen, n_samples, train_cache_dir, val_cache_dir):
    os.makedirs(train_cache_dir, exist_ok=True)
    os.makedirs(val_cache_dir, exist_ok=True)

    tokenizer = LlamaTokenizer.from_pretrained(base_model, use_fast=False)
    fp_model = AutoModelForCausalLM.from_pretrained(base_model, torch_dtype=torch.float16, cache_dir=HF_CACHE_DIR,
                                                    device_map="auto")
    fp_model.eval()
...
if __name__ == '__main__':
    ...
    cache_distillation_outputs(args.base_model, args.seqlen, args.n_samples, args.train_cache_dir, args.val_cache_dir)
