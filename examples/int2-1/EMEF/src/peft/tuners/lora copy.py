...
if is_gptq_available():
    class LinearqbitLt(QuantLinear, LoraLayer):
        ...
        def reset_parameters(self):
            if hasattr(self, "lora_A"):
                # initialize A the same way as the default for nn.Linear and B to zero
                # self.lora_A.weight.data = nn.init.kaiming_uniform(self.lora_A.weight.data, a=math.sqrt(5))
                nn.init.kaiming_uniform_(self.lora_A.weight, a=math.sqrt(5))
                # Set all lora_A weights to 0,5
                self.lora_A.weight = torch.nn.Parameter(torch.ones_like(self.lora_A.weight.data) * 0.5)
                self.lora_A.weight.requires_grad = True
                nn.init.zeros_(self.lora_B.weight)

        def zeros_grad(self, grad_output):
        ...
        # ):
        #     # super().backward = self.zeros_grad
        #     if self.r > 0:
        #         if not torch.is_autocast_enabled():
        #             expected_dtype = grad_output.dtype
        #             if grad_output.dtype != torch.float32:
        #                 grad_output = grad_output.float()
        #             grad_output = self.lora_A.backward(self.lora_B.backward(grad_output / self.scaling)).to(expected_dtype)
        #         else:
        #             grad_output = self.lora_A.backward(self.lora_B.backward(grad_output / self.scaling))
