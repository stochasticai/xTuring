...
def unpack_qweight(qweight, scales, qzeros, g_idx, bits, maxq, wf):
    if bits in [2,4,8]:
    ...
    num_itr = g_idx.shape[0]//(qweight.shape[0] * 32 / bits)
    if num_itr == 1:
...
def matmul3(input, qweight, scales, qzeros, g_idx, bits, maxq, wf):
    #print('input', input.device, 'qweight', qweight.device, 'scales', scales.device, 'qzeros', qzeros.device, 'g_idx', g_idx.device, 'wf', wf.device)
    #print('forward', g_idx.shape[0]//input.shape[-1], g_idx.shape[0], input.shape, qweight.shape)
    weight = unpack_qweight(qweight, scales, qzeros, g_idx, bits, maxq, wf.to(qzeros.device))
    output = torch.matmul(input, weight)
    return output

def transpose_matmul3(input, qweight, scales, qzeros, g_idx, bits, maxq, wf):
    #print('input', input.device, 'qweight', qweight.device, 'scales', scales.device, 'qzeros', qzeros.device, 'g_idx', g_idx.device, 'wf', wf.device)
    #print('backward', g_idx.shape[0]//input.shape[-1], g_idx.shape[0], input.shape, qweight.shape)
    weight = unpack_qweight(qweight, scales, qzeros, g_idx, bits, maxq, wf.to(qzeros.device))
...
class QuantLinearFunction(torch.autograd.Function):
    ...
    def forward(ctx, input, qweight, scales, qzeros, g_idx, bits, maxq, wf):
        if bits in [2,4,8]:
            output = matmul248(input, qweight, scales, qzeros, g_idx, bits, maxq)
        elif bits==3:
            output = matmul3(input, qweight, scales, qzeros, g_idx, bits, maxq, wf)
        else:
    ...
    def backward(ctx, grad_output):
        ...
        if ctx.needs_input_grad[0]:
            if bits in [2,4,8]:
                ...
                grad_input = transpose_matmul3(grad_output, qweight, scales, qzeros, g_idx, bits, maxq, wf)
            else:
                raise NotImplementedError("Only 2,3,4,8 bits are supported.")
...
class QuantLinear(nn.Module): 
    ...
    def forward(self, x):
        ...
        out = QuantLinearFunction.apply(x.reshape(-1,x.shape[-1]), self.qweight, self.scales, 
                                        self.qzeros, self.g_idx, self.bits, self.maxq, self.wf)
