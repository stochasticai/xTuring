...
class TextDataset(Dataset):
    ...
    def _validate(self):
        ...
        if len(self.data["train"].column_names) > 1:
            assert "target" in self.data["train"].column_names, "The dataset should have a column named target if there is more than one column"
            assert len(self.data["train"].column_names) == 2, "The dataset should have only two columns, text and target"
